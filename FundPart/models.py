...
class UrlPublish(models.Model):
    ...
    urlreadcount = models.IntegerField(default=0, editable=False,verbose_name=u'阅读量')

    class Meta:
        ordering = ['-urlpublish_time']

    def __unicode__(self):
        return u'%s' % (self.urlintroduce)
...
class ArticlePublish(models.Model):
    ...
    def __unicode__(self):
        return u'%s %s' %(self.usernameid,self.publish_time)
